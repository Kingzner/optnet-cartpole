import os
import sys

# Allow this script to be run directly from the repository root, e.g.
#     python scripts/train/train_cartpole.py
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

import torch
import numpy as np
from torch.utils.data import DataLoader, TensorDataset
import torch.nn as nn
import torch.optim as optim
import argparse

from cartpole.models import OptNetCartPoleMPC

###############################
# 命令行参数解析
###############################
parser = argparse.ArgumentParser(description='OptNet CartPole MPC Training')
parser.add_argument('--N', type=int, default=10, help='MPC prediction horizon (default: 10)')
parser.add_argument('--u-max', type=float, default=10.0, help='Control input max magnitude (default: 10.0)')
parser.add_argument('--u-min', type=float, default=-10.0, help='Control input min magnitude (default: -10.0)')
parser.add_argument('--x-max', type=float, default=1.0, help='Position constraint upper bound (default: 1.0)')
parser.add_argument('--x-min', type=float, default=-1.0, help='Position constraint lower bound (default: -1.0)')
parser.add_argument('--reg', type=float, default=1.0, help='QP regularization epsilon (default: 1.0)')
parser.add_argument('--dt', type=float, default=0.01, help='Discrete time step used by the MPC model (default: 0.01)')
parser.add_argument('--dynamics-file', type=str, default=None, help='Optional .npz file with A_d and B_d for MPC dynamics')
parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate (default: 1e-3)')
parser.add_argument('--epochs', type=int, default=50, help='Number of training epochs (default: 50)')
parser.add_argument('--batch-size', type=int, default=32, help='Batch size (default: 32)')
parser.add_argument('--output-dir', type=str, default='outputs/main', help='Output directory')
parser.add_argument('--data-dir', type=str, default='data', help='Data directory')
args = parser.parse_args()

BATCH_SIZE = args.batch_size
NUM_EPOCHS = args.epochs
LR = args.lr
WORKDIR = args.output_dir
os.makedirs(WORKDIR, exist_ok=True)

def load_data(data_dir="data", normalize=True, val_ratio=0.1, test_ratio=0.2):
    X = torch.load(os.path.join(data_dir, "features.pt"))
    Y = torch.load(os.path.join(data_dir, "labels.pt"))

    X = X.unsqueeze(-1).float()
    Y = Y.unsqueeze(-1).float()

    N = X.shape[0]

    perm = torch.randperm(N)
    X = X[perm]
    Y = Y[perm]

    n_train = int(N * (1.0 - val_ratio - test_ratio))
    n_val = int(N * val_ratio)

    X_train = X[:n_train]
    Y_train = Y[:n_train]
    X_val = X[n_train:n_train + n_val]
    Y_val = Y[n_train:n_train + n_val]
    X_test = X[n_train + n_val:]
    Y_test = Y[n_train + n_val:]

    if normalize:
        x_mean = X_train.mean(dim=(0, 1), keepdim=True)
        x_std = X_train.std(dim=(0, 1), keepdim=True) + 1e-8
        y_mean = Y_train.mean(dim=(0, 1), keepdim=True)
        y_std = Y_train.std(dim=(0, 1), keepdim=True) + 1e-8
        norm_stats = {"x_mean": x_mean, "x_std": x_std, "y_mean": y_mean, "y_std": y_std}
    else:
        norm_stats = None

    train_loader = DataLoader(TensorDataset(X_train, Y_train), batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(TensorDataset(X_val, Y_val), batch_size=BATCH_SIZE, shuffle=False)
    test_loader = DataLoader(TensorDataset(X_test, Y_test), batch_size=BATCH_SIZE, shuffle=False)

    return train_loader, val_loader, test_loader, norm_stats

def train_epoch(model, loader, optimizer, device):
    model.train()
    total_loss = 0
    n_samples = 0
    for X, Y in loader:
        X = X.to(device)
        Y = Y.to(device)
        optimizer.zero_grad()
        pred = model(X)
        loss = nn.MSELoss()(pred, Y)
        loss.backward()
        optimizer.step()
        total_loss += loss.item() * X.shape[0]
        n_samples += X.shape[0]
    return total_loss / n_samples

def eval_epoch(model, loader, device):
    model.eval()
    total_loss = 0
    n_samples = 0
    with torch.no_grad():
        for X, Y in loader:
            X = X.to(device)
            Y = Y.to(device)
            pred = model(X)
            loss = nn.MSELoss()(pred, Y)
            total_loss += loss.item() * X.shape[0]
            n_samples += X.shape[0]
    return total_loss / n_samples

def evaluate_metrics(model, loader, device, norm_stats, split_name=''):
    y_mean = norm_stats['y_mean'].to(device)
    y_std = norm_stats['y_std'].to(device)

    all_pred = []
    all_true = []

    with torch.no_grad():
        for x, y in loader:
            x = x.to(device)
            y = y.to(device)
            pred = model(x)

            y_denorm = y * y_std + y_mean
            pred_denorm = pred * y_std + y_mean

            all_true.append(y_denorm.view(-1))
            all_pred.append(pred_denorm.view(-1))

    all_true = torch.cat(all_true)
    all_pred = torch.cat(all_pred)

    mse = torch.mean((all_pred - all_true) ** 2).item()
    rmse = mse ** 0.5
    mae = torch.mean(torch.abs(all_pred - all_true)).item()

    ss_res = torch.sum((all_true - all_pred) ** 2)
    ss_tot = torch.sum((all_true - torch.mean(all_true)) ** 2)
    r2 = (1 - ss_res / (ss_tot + 1e-8)).item()

    vx = all_true - torch.mean(all_true)
    vy = all_pred - torch.mean(all_pred)
    corr = (torch.sum(vx * vy) / (torch.sqrt(torch.sum(vx ** 2) + 1e-8) * torch.sqrt(torch.sum(vy ** 2) + 1e-8))).item()

    print(f"=== {split_name} metrics (denormalized) ===")
    print(f"MSE : {mse:.6f}")
    print(f"RMSE: {rmse:.6f}")
    print(f"MAE : {mae:.6f}")
    print(f"R^2 : {r2:.6f}")
    print(f"Corr: {corr:.6f}")
    print()

    return {"MSE": mse, "RMSE": rmse, "MAE": mae, "R2": r2, "Corr": corr}

def main():
    torch.manual_seed(42)
    np.random.seed(42)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(42)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"Parameters: N={args.N}, u_max={args.u_max}, x_max={args.x_max}, reg={args.reg}, dt={args.dt}, dynamics_file={args.dynamics_file}")

    train_loader, val_loader, test_loader, norm_stats = load_data(args.data_dir, normalize=True)

    torch.save(norm_stats, os.path.join(WORKDIR, "norm_stats.pt"))

    model = OptNetCartPoleMPC(
        N=args.N,
        u_min=args.u_min,
        u_max=args.u_max,
        x_min=args.x_min,
        x_max=args.x_max,
        q_reg_eps=args.reg,
        dt=args.dt,
        dynamics_file=args.dynamics_file
    ).to(device)

    model.set_norm_stats(norm_stats, device=device)

    optimizer = optim.Adam(model.parameters(), lr=LR, weight_decay=1e-4)

    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode="min",
        factor=0.5,
        patience=5,
        verbose=True
    )

    f_train = open(os.path.join(WORKDIR, "train.csv"), "w")
    f_val = open(os.path.join(WORKDIR, "val.csv"), "w")
    f_test = open(os.path.join(WORKDIR, "test.csv"), "w")
    f_train.write("epoch,loss\n")
    f_val.write("epoch,loss\n")
    f_test.write("epoch,loss\n")

    print("=== Start Training ===")

    best_val = float("inf")

    for epoch in range(1, NUM_EPOCHS + 1):
        train_loss = train_epoch(model, train_loader, optimizer, device)
        val_loss = eval_epoch(model, val_loader, device)
        test_loss = eval_epoch(model, test_loader, device)

        print(f"[{epoch:03d}] Train={train_loss:.4f} | Val={val_loss:.4f} | Test={test_loss:.4f}")

        f_train.write(f"{epoch},{train_loss}\n")
        f_val.write(f"{epoch},{val_loss}\n")
        f_test.write(f"{epoch},{test_loss}\n")
        f_train.flush()
        f_val.flush()
        f_test.flush()

        scheduler.step(val_loss)

        if val_loss < best_val:
            best_val = val_loss
            torch.save(
                model.state_dict(),
                os.path.join(WORKDIR, "cartpole_optnet_best_2B.pth")
            )

    f_train.close()
    f_val.close()
    f_test.close()

    best_model_path = os.path.join(WORKDIR, "cartpole_optnet_best_2B.pth")
    model.load_state_dict(torch.load(best_model_path, map_location=device))

    print("=== Final evaluation on denormalized control u ===")
    evaluate_metrics(model, train_loader, device, norm_stats, split_name="Train")
    evaluate_metrics(model, val_loader, device, norm_stats, split_name="Val")
    evaluate_metrics(model, test_loader, device, norm_stats, split_name="Test")

    params_file = os.path.join(WORKDIR, "training_params.txt")
    with open(params_file, 'w') as f:
        f.write(f"N={args.N}\n")
        f.write(f"u_max={args.u_max}\n")
        f.write(f"x_max={args.x_max}\n")
        f.write(f"reg={args.reg}\n")
        f.write(f"lr={args.lr}\n")
        f.write(f"epochs={args.epochs}\n")

    print(f"\nTraining complete! Results saved to {WORKDIR}")

if __name__ == "__main__":
    main()
